...
| `src/ukrmap.js` — the same code, annotated | 93.1 KB | 30.7 KB |
...
**The Dnipro stops at Kherson** for the same reason, and the gap below it is worth knowing about, because it is a real place and not a defect. Past the city the river spreads into the **Oleshky floodplain** — Олешківські плавні, a delta of dozens of branches, marsh islands and oxbows some 25 km across, which is where the Dnipro arrives at the Dnipro–Buh estuary. There is no single channel to draw through it: Natural Earth's centerline ends at the head of the delta, the marsh itself is mapped as land, and the estuary beyond it is sea. So the thread stops about 200 map units short of open water, and those 200 units are the delta. Like everything else wet here, that is the shape before June 2023; what the floodplain became after the Kakhovka dam was destroyed — first flooded, then drained with the reservoir above it — is not in any surveyed geometry yet.
